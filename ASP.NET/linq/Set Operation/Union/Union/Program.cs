...
namespace Union
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var ms1 = students1.Select(x => x.Name).Union(students2.Select(x => x.Name)).ToList();
            var ms2 = students1.Select(x => new { x.Id, x.Name }).Union(students2.Select(x => new { x.Id, x.Name })).ToList();
            var ms3 = students1.Union(students2, new comparetor()).ToList();

            var qs1 = (from std in students1
                       select std).Union(students2, new comparetor()).ToList();
        }
    }
