...
namespace intersect
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var ms1 = students1.Select(x => x.Name).Intersect(students2.Select(x => x.Name)).ToList();
            var ms2 = students1.Select(x => new { x.Id, x.Name }).Intersect(students2.Select(x => new { x.Id, x.Name })).ToList();
            var ms3 = students1.Intersect(students2, new comparetor()).ToList();

            var qs1 = (from std in students1
                       select std).Intersect(students2, new comparetor()).ToList();
        }
    }
